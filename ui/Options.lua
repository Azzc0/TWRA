...
function TWRA:CreateOptionsInMainFrame()
    ...
    -- OSD Enable checkbox (ADD THIS FIRST) 
    ...
    -- NEW: OSD on Navigation checkbox
    ...
    -- Lock Position checkbox (MOVED DOWN)
    local lockOSD = CreateFrame("CheckButton", nil, middleColumn, "UICheckButtonTemplate")
    lockOSD:SetPoint("TOPLEFT", showOnNavOSD, "BOTTOMLEFT", 0, -10) -- Changed to position below enableOSD
    lockOSD:SetWidth(24)
    ...
    tankSyncCheckbox:SetScript("OnClick", function()
        ...
        local isChecked = tankSyncCheckbox:GetChecked()
    ...
    if not liveSyncEnabled or not oRA2Available then
        self:SafeToggleUIElement(tankSyncCheckbox, false, tankSyncText)
    else
        self:SafeToggleUIElement(tankSyncCheckbox, true, tankSyncText)
    end
    ...
    autoNavigate:SetScript("OnClick", function()
        ...
        if not superWowAvailable then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("TWRA: AutoNavigate requires SuperWoW API, which is not available.")
            return
        end
        
        -- Toggle AutoNavigate without opening main window
        local isChecked = autoNavigate:GetChecked()
        ...
        -- Call the specific toggle function if it exists
        if self.ToggleAutoNavigate then
            self:ToggleAutoNavigate(isChecked)
        end
    end)
    ...
    if not superWowAvailable then
        ...
        getglobal(scanSlider:GetName() .. "High"):SetTextColor(0.5, 0.5, 0.5)
        scanSlider:EnableMouse(false)
    elseif not autoNavEnabled then
        getglobal(scanSlider:GetName() .. "Text"):SetTextColor(0.5, 0.5, 0.5)
        getglobal(scanSlider:GetName() .. "Low"):SetTextColor(0.5, 0.5, 0.5)
        getglobal(scanSlider:GetName() .. "High"):SetTextColor(0.5, 0.5, 0.5)
        scanSlider:EnableMouse(false)
    ...
    enableOSD:SetScript("OnClick", function()
        local isChecked = enableOSD:GetChecked()
    ...
    showOnNavOSD:SetScript("OnClick", function()
        local isChecked = showOnNavOSD:GetChecked()
        TWRA_SavedVariables.options.osd = TWRA_SavedVariables.options.osd or {}
    ...
    lockOSD:SetScript("OnClick", function()
        local isChecked = lockOSD:GetChecked()
        TWRA_SavedVariables.options.osd = TWRA_SavedVariables.options.osd or {}
...
function TWRA:ToggleOptionsPanel()
    ...
    if self.currentView == "options" then
        -- If we're in options view, switch to main view
        self:ShowMainView()
    else
        -- If we're in main view, switch to options view
