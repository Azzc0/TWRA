...
-- Show options view (hide main view)
function TWRA:ShowOptionsView()
    ...
    if self.navigation then
        ...
        if self.navigation.handlerText then 
            if self.navigation.handlerText:GetParent() then
                self.navigation.handlerText:GetParent():Hide() 
            end
        end
...
-- Fix ShowMainView function to properly restore data and UI state
function TWRA:ShowMainView()
    -- Set the current view state first
    ...
    if self.navigation then
        ...
        if self.navigation.handlerText and self.navigation.handlerText:GetParent() then
            self.navigation.handlerText:GetParent():Show() 
        end
    end
    ...
    -- Set the button text back to "Options"
    ...
    if self.pendingNavigation then
        ...
        -- Force a redisplay of the current section
        self:DisplayCurrentSection()
    end
    
    -- Recalculate frame height after restoration
    if self.fullData and self.rowFrames then
        local totalHeight = 40 +                                    -- Initial offset
                            (table.getn(self.rowFrames) * 20) +     -- Data rows
                            25                                      -- Footer padding
        
        -- Add height for footers if they exist
        if self.footers and table.getn(self.footers) > 0 then
            totalHeight = totalHeight + ((table.getn(self.footers) - 1) * 28) + 10
        end
    ...
    self:Debug("ui", "Restored main view")
end
...
function TWRA:CreateOptionsInMainFrame()
    ...
    importBtn:SetScript("OnClick", function()
        ...
        DEFAULT_CHAT_FRAME:AddMessage("TWRA: Starting import process with " .. string.len(importText) .. " characters")
        
        -- Enable data debugging temporarily
        TWRA:ToggleDebugCategory("data", true)
        
        -- Direct call to Base64 decoder - no pcall wrapper
        ...
        if not decodedData then
            DEFAULT_CHAT_FRAME:AddMessage("TWRA ERROR: Base64 decoding failed")
            return
        end
        
        DEFAULT_CHAT_FRAME:AddMessage("TWRA: Decoding successful, saving assignments")
        
        -- Save the data with the original import string
        TWRA:SaveAssignments(decodedData, importText)
        
        -- Restore view and notify user
        DEFAULT_CHAT_FRAME:AddMessage("TWRA: Assignment data imported successfully")
        importBox:SetText("")
        
        -- If we were previously viewing a section, try to restore it
        if TWRA.currentView == "options" then
            -- If we're in options view, set the pendingNavigation flag
            TWRA.pendingNavigation = TWRA.navigation and TWRA.navigation.currentIndex or 1
        else
            -- Otherwise update the view immediately
    ...
    autoNavigate:SetChecked(self.AUTONAVIGATE and self.AUTONAVIGATE.enabled or false)
    autoNavigate:SetScript("OnClick", function()
        -- Check if the function exists before calling it
        if self.ToggleAutoNavigate then
            -- Toggle AutoNavigate without opening main window
            self:ToggleAutoNavigate(autoNavigate:GetChecked())
        else
            -- Function not available, update directly
            if not self.AUTONAVIGATE then
                self.AUTONAVIGATE = {
                    enabled = false,
                    scanFreq = 3
            ...
            -- Update UI state based on the new setting
...
-- Import function to handle the decoding and saving process
function TWRA:ImportAssignments(base64Str)
    if not base64Str or base64Str == "" then
        self:Error("No import data provided")
        return nil
    end
    
    self:Debug("ui", "Starting import process with " .. string.len(base64Str) .. " characters")
    
    -- Enable data debugging temporarily if not already enabled
    local wasDataDebugEnabled = self:IsDebugging("data") 
    if not wasDataDebugEnabled then
        self:ToggleDebugCategory("data", true)
    end
    
    -- Call decode function directly
    local result = self:DecodeBase64(base64Str)
    ...
    if not result then
        self:Debug("general", "Base64 decoding failed")
        return nil
    end
    
    -- Save the assignments
    self:SaveAssignments(result, base64Str)
    
    -- Return the decoded data
    return result
end

-- Add this additional helper function if it doesn't exist
function TWRA:IsDebugging(category)
    return self.DEBUG and self.DEBUG.enabled and self.DEBUG.categories and 
           self.DEBUG.categories[category or "general"]
end

TWRA:Debug("ui", "Options module loaded")