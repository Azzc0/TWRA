...
function TWRA:InitOSD()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("TWRA: OSD module initialized")
end

-- Enhanced function to display section overlay with relevant information
function TWRA:ShowSectionNameOverlay(sectionName, currentIndex, totalSections, persistent)
    DEFAULT_CHAT_FRAME:AddMessage("TWRA Debug: ShowSectionNameOverlay called with section: " .. 
                                 (sectionName or "nil") .. ", index: " .. 
                                 (currentIndex or "nil") .. ", total: " .. 
                                 (totalSections or "nil"))
    ...
    -- Update content if we have data and a section name
    if sectionName and self.fullData then
        ...
        -- Hide warning and note elements
        ...
        -- Set minimal height for the overlay
        self.sectionOverlay:SetHeight(100)
    end
            ...
            if not rowInfo.isPlayerHealer then
                ...
                if not rowInfo.isPlayerTank and table.getn(rowInfo.tanks) > 0 then
                    ...
                    for j, tank in ipairs(rowInfo.tanks) do
                        ...
                        if not tank.inRaid then
                            tankName:SetTextColor(1.0, 0.3, 0.3) -- Red for not in raid
                        elseif not tank.online then
                            tankName:SetTextColor(0.5, 0.5, 0.5) -- Gray for offline
                        elseif tank.class and self.VANILLA_CLASS_COLORS and self.VANILLA_CLASS_COLORS[tank.class] then
                            local color = self.VANILLA_CLASS_COLORS[tank.class]
                            tankName:SetTextColor(color.r, color.g, color.b) -- Class color
                        else
                            tankName:SetTextColor(1, 1, 1) -- White if no class color available
                        end
            ...
            if rowInfo.isPlayerHealer then
                ...
                if table.getn(rowInfo.tanks) > 0 then
                    for j, tank in ipairs(rowInfo.tanks) do
                        ...
                        -- Set color based on status
                        if not tank.inRaid then
                            tankText:SetTextColor(1.0, 0.3, 0.3) -- Red for not in raid
                        elseif not tank.online then
                            tankText:SetTextColor(0.5, 0.5, 0.5) -- Gray for offline
                        elseif tank.class and self.VANILLA_CLASS_COLORS and self.VANILLA_CLASS_COLORS[tank.class] then
                            local color = self.VANILLA_CLASS_COLORS[tank.class]
                            tankText:SetTextColor(color.r, color.g, color.b) -- Class color
                        else
                            tankText:SetTextColor(1, 1, 1) -- White if no class color available
                        end
                ...
                -- -- Add other healers if present
                -- if table.getn(rowInfo.healers) > 0 then
                --     -- Add "with" text
                --     local withText = lineContainer:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
                --     withText:SetPoint("LEFT", currentXPos, "RIGHT", 5, 0)
                --     withText:SetText("with")
                --     withText:SetTextColor(0.8, 0.8, 0.8) -- Slightly dimmer
                --     currentXPos = withText
                ...
                --         -- Add healer name
                --         local healerText = lineContainer:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
                --         healerText:SetPoint("LEFT", currentXPos, "RIGHT", 3, 0)
                --         healerText:SetText(healer.name)
                        
                --         -- Set color based on status
                --         if not healer.inRaid then
                --             healerText:SetTextColor(1.0, 0.3, 0.3) -- Red for not in raid
                --         elseif not healer.online then
                --             healerText:SetTextColor(0.5, 0.5, 0.5) -- Gray for offline
                --         elseif healer.class and self.VANILLA_CLASS_COLORS and self.VANILLA_CLASS_COLORS[healer.class] then
                --             local color = self.VANILLA_CLASS_COLORS[healer.class]
                --             healerText:SetTextColor(color.r, color.g, color.b) -- Class color
                --         else
                --             healerText:SetTextColor(1, 1, 1) -- White if no class color available
                --         end
                        
                --         currentXPos = healerText
                --     end
                -- end
            else
                ...
                if rowInfo.isPlayerTank then
                    ...
                    if table.getn(rowInfo.healers) > 0 then
                        ...
                        for j, healer in ipairs(rowInfo.healers) do
                            ...
                            -- Set color based on status
                            if not healer.inRaid then
                                healerName:SetTextColor(1.0, 0.3, 0.3) -- Red for not in raid
                            elseif not healer.online then
                                healerName:SetTextColor(0.5, 0.5, 0.5) -- Gray for offline
                            elseif healer.class and self.VANILLA_CLASS_COLORS and self.VANILLA_CLASS_COLORS[healer.class] then
                                local color = self.VANILLA_CLASS_COLORS[healer.class]
                                healerName:SetTextColor(color.r, color.g, color.b) -- Class color
                            else
                                healerName:SetTextColor(1, 1, 1) -- White if no class color available
                            end
...
-- Toggle OSD visibility (for keybinding)
function TWRA:ToggleOSD()
    -- If OSD doesn't exist yet, create it and show it persistently
    if not self.sectionOverlay then
        self:ShowSectionNameOverlay(nil, nil, nil, true) -- Pass true to make it persistent
    ...
    elseif self.sectionOverlay:IsShown() then
        self.sectionOverlay:Hide()
        DEFAULT_CHAT_FRAME:AddMessage("TWRA: OSD hidden")
    -- If OSD exists but is hidden, show it persistently
    else
        self:ShowSectionNameOverlay(nil, nil, nil, true) -- Pass true to make it persistent
        DEFAULT_CHAT_FRAME:AddMessage("TWRA: OSD shown (persistent)")
    end
...
-- This is now separate from the toggle function
function TWRA:TestOSD()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("TWRA: Testing OSD with section '" .. sectionName .. "'")
end
...
function TWRA:ResetOSDPosition()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("TWRA: OSD position reset to default")
