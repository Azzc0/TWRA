...
function TWRA:NavigateToSection(targetSection, suppressSync)
    -- Accept either section name or index
    local sectionIndex
    ...
    if not self.navigation or not self.navigation.handlers then
        DEFAULT_CHAT_FRAME:AddMessage("TWRA Debug: NavigateToSection - Navigation not initialized")
        return false
    end
    
    if not sectionIndex or sectionIndex < 1 or sectionIndex > table.getn(self.navigation.handlers) then
        DEFAULT_CHAT_FRAME:AddMessage("TWRA Debug: NavigateToSection - Invalid section index: " .. (sectionIndex or "nil"))
        return false
    end
    
    -- Get section name for displaying (important for sync)
    local sectionName = self.navigation.handlers[sectionIndex]
    if not sectionName then
        DEFAULT_CHAT_FRAME:AddMessage("TWRA Debug: NavigateToSection - Invalid section name for index: " .. sectionIndex)
        return false
    end
    ...
    -- Save the current section in saved variables
    ...
    if self.currentView == "options" then
        -- Just store the pending navigation for when we go back to main view
        DEFAULT_CHAT_FRAME:AddMessage("TWRA Debug: NavigateToSection - Deferring display update (in options view)")
        self.pendingNavigation = sectionIndex
    else
        -- Update display immediately since we're in main view
        DEFAULT_CHAT_FRAME:AddMessage("TWRA Debug: NavigateToSection - Updating display to " .. sectionName)
        self:DisplayCurrentSection()
        
        -- Show the section in OSD unless suppressed
        if not suppressSync and self.ShowSectionNameOverlay then
            self:ShowSectionNameOverlay(sectionName, sectionIndex, table.getn(self.navigation.handlers))
        end
    end
    
    -- Broadcast section change if not suppressed and we have sync enabled
    if not suppressSync and self.SYNC and self.SYNC.liveSync then
        DEFAULT_CHAT_FRAME:AddMessage("TWRA Debug: Broadcasting section change: " .. sectionName .. " (index " .. sectionIndex .. ")")
...
function TWRA:NavigateToSection(targetSection, suppressSync)
    ...
    -- Update UI - specifically update the section handler text
    if self.navigation.handlerText and sectionName then
        self.navigation.handlerText:SetText(sectionName)
    end
    
    -- Only filter and display the handler if we're not in options view
    if self.currentView ~= "options" then
        self:FilterAndDisplayHandler(sectionName)
    else
        -- If in options view, make sure we clean up any visible rows
        -- This prevents section content from appearing behind options UI
        self:ClearRows()
    end
    ...
    -- Show OSD if the main frame is not shown OR we're in options view
    -- This ensures OSD is shown when navigating with keybinds regardless of UI state
    if not self.mainFrame:IsShown() or self.currentView == "options" then
        if self.ShowSectionNameOverlay then
            self:ShowSectionNameOverlay(sectionName, sectionIndex, numSections)
        else
            DEFAULT_CHAT_FRAME:AddMessage("TWRA Error: ShowSectionNameOverlay function not found")
        end
    end
